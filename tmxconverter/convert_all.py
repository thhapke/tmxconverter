...
def main() : # encapsulated into main otherwise entrypoint is not working
    ...
    for i, filename in enumerate(tmxfiles):
        ...
        logging.info('{}/{}: {}'.format(i,max_number_files, filename))
        try :
            tree = ET.parse(path.join(input_folder, filename))
        except ET.ParseError as e:
            logging.warning('XML ParseError in file {} - {}'.format(filename, e))
            xmlstring = remedyxml(path.join(input_folder, filename),e.position)
